...
export default function CatalogManager() {
  ...
  // ─── US002-C: Helper que devuelve el nivel de alerta del stock ──────────────
  // NUEVO: Esta función centraliza la lógica de alertas visuales.
  // Retorna null si el stock está bien, "low" si está bajo, "out" si está agotado.
  const getStockAlert = (stock: number): "out" | "low" | null => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-[#1a1010] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm">
        ...
        {/* ─── US002-C: Contador de alertas activas en el header ─────────────────
            NUEVO: Muestra cuántos productos necesitan atención inmediata.
            Se calcula con filter + getStockAlert para no duplicar lógica.        */}
        {products.filter(p => getStockAlert(p.stock) !== null).length > 0 && (
          <div className="flex items-center gap-2 bg-orange-50 border border-orange-200 text-orange-600 px-4 py-2 rounded-xl text-sm font-bold">
            <span className="material-symbols-outlined text-lg">warning</span>
            {products.filter(p => getStockAlert(p.stock) !== null).length} producto(s) con stock bajo
          </div>
        )}
        ...
        <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {products.map((p) => (
                        // ─── US002-C: Fila con borde de alerta si el stock es crítico ─────
                        // NUEVO: La fila completa cambia de apariencia si hay alerta activa.
                        <tr
                          key={p.id}
                          className={`hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group ${
                            getStockAlert(p.stock) === "out"
                              ? "bg-red-50/40 dark:bg-red-900/10"
                              : getStockAlert(p.stock) === "low"
                        ...
                        >
                            ...
                            <td className="p-4">
                                <div className="flex flex-col gap-1.5">
                                  ...
                                  {/* ─── US002-C: Badge de alerta visual ─────────────────────────────
                                      NUEVO: Aparece debajo del control de stock cuando el nivel
                                      es bajo (≤5) o agotado (0). Cumple la historia US002-C
                                      mostrando la alerta directamente al vendedor en el catálogo. */}
                                  {getStockAlert(p.stock) === "out" && (
                                    <span className="flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-100 border border-red-200 px-2 py-0.5 rounded-full w-fit">
                                      <span className="material-symbols-outlined text-xs">block</span>
                                      Agotado
                                    </span>
                                  )}
                                  {getStockAlert(p.stock) === "low" && (
                                    <span className="flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-100 border border-orange-200 px-2 py-0.5 rounded-full w-fit">
                            ...
                            <td className="p-4 text-right">
                                <div className="flex items-center justify-end gap-2">
                                    <button onClick={() => handleEditClick(p)} className="size-8 flex items-center justify-center rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors" title="Editar">
                                        <span className="material-symbols-outlined text-lg">edit</span>
                                    </button>
                                    <button onClick={() => handleDelete(p.id)} className="size-8 flex items-center justify-center rounded-full bg-red-50 text-red-500 hover:bg-red-100 transition-colors" title="Eliminar permanentemente">
